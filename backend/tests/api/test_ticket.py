...
async def test_sut_use_ticket_returns_expired_status_when_old_ticket_exists(
    authorized_client: AsyncClient,
    dummy_lecture: Lecture,
    ...
    """만료된 ticket_usage(>1주)가 있을 때 재사용 시도 → 500이 아닌 200 accessible=False, reason=ticket_expired"""
    from app.db.tables import User as UserTable
    ...
    data = response.json()
    assert data["accessible"] is False
    assert data["reason"] == "ticket_expired"
    assert data["expires_at"] is not None

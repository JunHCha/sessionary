...
describe('LoginModal', () => {
	describe('Props 기본값', () => {
		it('message 기본값은 "로그인이 필요합니다"이다', async () => {
			...
			const { getDefaultMessage } = await import('./LoginModal.svelte')
			expect(getDefaultMessage()).toBe('로그인이 필요합니다')
		})

		it('redirectUrl 기본값은 "/home"이다', async () => {
			// LoginModal 컴포넌트의 기본 redirectUrl prop 확인
			const { getDefaultRedirectUrl } = await import('./LoginModal.svelte')
	...
	describe('sessionStorage 저장', () => {
		it('saveRedirectUrl 호출 시 sessionStorage에 redirectUrl이 저장된다', async () => {
			const { saveRedirectUrl } = await import('./LoginModal.svelte')
		...
		it('redirectUrl이 없으면 기본값 "/home"이 저장된다', async () => {
			const { saveRedirectUrl, getDefaultRedirectUrl } = await import('./LoginModal.svelte')
